...
const riskyPatterns = [
  ...
  { label: "access code", pattern: /(lockbox|alarm|access code|zugangscode|schluesselcode)\s*[:=]\s*\S+/i },
  { label: "private env value", pattern: /(api_key|secret|token|password)\s*=\s*['"]?[A-Za-z0-9_\-]{12,}/i }
];
...
async function walk(dir) {
  ...
  for (const entry of entries) {
    ...
    } else if (/\.(md|json|mjs|js|ts|tsx|yml|yaml|txt)$/.test(entry.name)) {
      files.push(full);
    }
  }
  return files;
